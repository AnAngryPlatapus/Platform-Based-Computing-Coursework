...
public class BinarySearchTree<T> implements BSTInterface<T>
{
    ...
    // Returns the number of elements in this BST.
    {
        ...
        if (root != null)
        {
            SuperLStack<BSTNode<T>> nodeStack = new SuperLStack<BSTNode<T>>();
            BSTNode<T> currNode;
            nodeStack.push(root);
...
*/
    ...
    //recursive method numLeaves that returns an int indicating the number
    //of leaves in the tree
    private int recNumLeaves(BSTNode<T> parent) {
        if (parent == null) //No nodes in tree, basecase
            return 0;
        // no child nodes, node must be a leaf basecase hit once tree is recursed down
        else if(parent.getLeft() == null && parent.getRight() == null)
            return 1;
        else
            // return the sum of the left and right until prior base case is hit
            return recNumLeaves(parent.getLeft()) + recNumLeaves(parent.getRight());
    }
    ...
    //recursive method that returns an int indicating showing the number of nodes
    //with exactly one child
    private int recNumSingPars(BSTNode<T> parent){
        if (parent == null) //No nodes in tree, basecase
            return 0;
        // node has two children, calls recursive function again
        else if((parent.getLeft() !=null && parent.getRight() !=null))
            return recNumSingPars(parent.getLeft()) + recNumSingPars(parent.getRight());
        // node has one right child, add and recurse down the child
        else if((parent.getLeft() == null) && (parent.getRight() !=null))
            return 1 + recNumSingPars(parent.getRight());
        // node has one left child, add and recurse down the child
        else if((parent.getLeft() != null) && (parent.getRight() == null))
            return 1 + recNumSingPars(parent.getLeft());
        else
            return 0;


    }
    public int numSingPars() {
        return recNumSingPars(root);
    }
}